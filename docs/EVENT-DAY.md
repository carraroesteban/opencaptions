# Día del evento — runbook de producción
...
1. **Server**: una VM chica (2 vCPU / 2 GB alcanzan para 10+ salas) o una notebook dedicada.
   ...
   cp .env.example .env         # GEMINI_API_KEY, INGEST_TOKEN, ADMIN_TOKEN, PUBLIC_URL
...
| Ingesta (mini PC) | `/ingest.html?stage=<sala>&token=<INGEST_TOKEN>&autostart=1` |
