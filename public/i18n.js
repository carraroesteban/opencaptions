...
const DICTS = {
  en: {
    ...
    '📅 Agenda': '📅 Agenda',
    '🖨 Kit de QR': '🖨 QR kit', '📌 Flotante': '📌 Float', 'Segunda línea (bilingüe)': 'Second line (bilingual)', 'Ninguna': 'None', 'Desde Swapcard, Sessionize o una planilla:': 'From Swapcard, Sessionize or a spreadsheet:', 'exportá las sesiones a Excel o Google Sheets, seleccioná todo (con la fila de títulos), copiá y pegá acá. Se reconocen las columnas (título, inicio, sala/ubicación, speakers) y las salas por su nombre; las filas de salas sin subtítulos se ignoran.': 'export the sessions to Excel or Google Sheets, select everything (including the header row), copy and paste here. Columns (title, start, room/location, speakers) and rooms are recognized by name; rows for rooms without captions are ignored.', 'O a mano, una charla por línea:': 'Or by hand, one talk per line:', '(hora': '(time', '= hoy).': '= today).', 'Para qué sirve: cada sala nombra sola sus charlas (pantallas, transcripciones y descargas) sin cortar a un speaker que se pasa de tiempo, y los nombres de speakers y títulos ayudan a la IA a escribirlos bien.': 'Why: each room names its talks by itself (screens, transcripts and downloads) without cutting off a speaker who runs late, and speaker names and titles help the AI spell them right.', 'Todas las salas en una ventana siempre visible, encima de OBS o vMix': 'Every room in an always-on-top window, over OBS or vMix',
