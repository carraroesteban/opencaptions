...
const eventName = await ask('Event name', evOld.eventName || 'My Conference');
console.log(dim('\nGet a Gemini API key at https://aistudio.google.com/apikey (leave empty to try it in simulated mode).'));
const apiKey = await ask('Gemini API key', envOld.GEMINI_API_KEY ? '•••• keep current' : '');
const roomsDefault = (evOld.stages || []).map((s) => s.name).join(', ') || 'Main stage, Room A';
const rooms = (await ask('Rooms, comma-separated', roomsDefault)).split(',').map((s) => s.trim()).filter(Boolean);
console.log(dim('\nTalk language: "auto" detects it; pin it (e.g. "en") when you know it — faster and more accurate.'));
const source = await ask('Language the talks are usually given in (auto / es / en / pt …)', 'auto');
const targets = (await ask('Caption languages for the audience, comma-separated', (evOld.defaultTargets || ['es', 'en']).join(','))).split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
...
const publicUrl = (await ask('Public URL', envOld.PUBLIC_URL || '')).replace(/\/$/, '');
const publicTranscripts = (await yes('Let the audience read and download transcripts of past talks too?', (evOld.publicTranscripts || 'all') === 'all')) ? 'all' : 'current';
const tz = await ask('Event time zone (for the agenda)', evOld.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone);
...
if (!(await yes('\nWrite .env and config/event.json?'))) { console.log('Nothing written.'); process.exit(0); }
...
const base = publicUrl || 'http://localhost:8080';
console.log(`\n${green('✓ Saved.')} ${dim('(previous files kept as .bak)')}\n`);
console.log(b('Next steps'));
console.log(`  1. Start the server:            ${b('npm start')}`);
console.log(`  2. Dashboard on this computer:  ${base.startsWith('http://localhost') ? 'http://localhost:8080/admin.html' : `${base}/admin.html?token=${adminToken}`}`);
...
console.log(`  5. Optional: paste the agenda in the dashboard (📅 Agenda) so talks get their titles automatically.`);
